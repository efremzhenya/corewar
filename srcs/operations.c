...
/*   Updated: 2021/01/04 19:23:37 by lseema           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_operations(t_op	*op)
{
	...
	op[3] = (t_op){"st", 2, {T_REG, T_IND | T_REG}, 3, 5, "store", 1, 0};
	op[4] = (t_op){"add", 3, {T_REG, T_REG, T_REG}, 4, 10, "addition", 1, 0};
	...
	op[9] = (t_op){"zjmp", 1, {T_DIR}, 9, 20, "jump if zero", 0, 1};
	op[10] = (t_op){"ldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 10,
		25, "load index", 1, 1};
	op[11] = (t_op){"sti", 3, {T_REG, T_REG | T_DIR | T_IND, T_DIR | T_REG}, 11,
		25, "store index", 1, 1};
	op[12] = (t_op){"fork", 1, {T_DIR}, 12, 800, "fork", 0, 1};
	op[13] = (t_op){"lld", 2, {T_DIR | T_IND, T_REG}, 13, 10, "long load", 1, 0};
	op[14] = (t_op){"lldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 14,
		50, "long load index", 1, 1};
	op[15] = (t_op){"lfork", 1, {T_DIR}, 15, 1000, "long fork", 0, 1};
	op[16] = (t_op){"aff", 1, {T_REG}, 16, 2, "aff", 1, 0};
	op[17] = (t_op){0, 0, {0}, 0, 0, 0, 0, 0};
}
